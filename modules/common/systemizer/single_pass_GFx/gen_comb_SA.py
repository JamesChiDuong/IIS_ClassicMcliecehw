...
print("""module {3} (
  ...
  output wire r_A_and
);""".format((num*(2+args.gf))-1, (num*args.gf)-1, num-1, args.name))
...
""".format((num*(2+args.gf))-1, (num*args.gf)-1))
...
print("  wire [{0}:0] data_delay;".format((num*args.gf)-1))
...
for rb in range(0, num, args.block):
  print("""  delay #(.WIDTH({3}), .DELAY({0})) del_inst_in_{0} (
    ...
    .dout(data_delay[{1}:{2}])
           ...
           min(rb+args.block, num)*args.gf - 1,
           rb*args.gf, 
           (min(rb+args.block, num) - rb)*args.gf))
...
for row in range(num):
  ...
  wire [{2}:0] fac_out_{0}_{1};\n""".format(row, col, args.gf-1))
    ...
    if row > 0:
      print("  reg  [{2}:0] data_in_{0}_{1} = 1'b0;".format(row, col, args.gf-1))
      ...
      print("  wire [{2}:0] data_in_{0}_{1};".format(row, col, args.gf-1))
      print("  wire [{2}:0] data_out_{0}_{1};\n".format(row, col, args.gf-1))

      print("  assign data_in_0_{0} = data_delay[{1}:{2}];".format(col, ((col+1)*args.gf)-1, (col*args.gf)))
    ...
    if col > 0:
      ...
      if col == block:
        ...
        print("      op_in_{0}_0 <= op_in_del[{1}:{2}];".format(row, (2+args.gf)*row+1, (2+args.gf)*row))
        print("      fac_in_{0}_0 <= op_in_del[{1}:{2}];".format(row, 
                   (2+args.gf)*(row+1)-1, (2+args.gf)*row+2))
        print("    end")
      else:
        if row == 0:
        ...
        print("  assign op_in_{0}_0 = op_in_del[{1}:{2}];".format(row, (2+args.gf)*row+1, (2+args.gf)*row))
    ...
    if row == (col):
      if row == 0:
         finish = "finish_del"
         inv_in = "data[{0}:0]".format(args.gf-1)
      else:
  ...
  wire inv_en_{0}_{1};\n""".format(row, col, args.gf-1))
      ...
      if not inv_suffix:
        ...
        print("""  gf_inv #(.DELAY(1)) gfe_inv_inst_{0}_{1}
  (
    .clk(clk),
    .din_A({2}),
...
print("")
print("  reg [{0}:0] data_out_reg = 0;".format((num*args.gf)-1))
print("")
