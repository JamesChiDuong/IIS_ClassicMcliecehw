...
def randombits(r):
    return [random.randrange(2) for j in range(r)]
...
class McElieceIO:
    ...
    def write_e(self, error, f_error=None, slicewidth=None):
        ...
        with open(f_error, 'w') as f:
            if slicewidth is None:
                ...
                error_ = error
                if self.params.n % slicewidth != 0:
                    remainder = self.params.n % slicewidth
                    error_ += [0 for i in range(slicewidth-remainder)]
                for i in range(0, self.params.n, slicewidth):
    ...
    def write_C(self, C,
        ...
        with open(f_cipher_0, 'w') as f:
            if slicewidth is None:
                ...
                c0_ = c0
                if len(c0_) % slicewidth != 0:
                    remainder = len(c0_) % slicewidth
                    c0_ += [0 for i in range(slicewidth-remainder)]
                for i in range(0, len(c0_), slicewidth):
        ...
        with open(f_cipher_1, 'w') as f:
            if slicewidth is None:
                ...
                c1_ = c1
                if len(c1_) % slicewidth != 0:
                    remainder = len(c1_) % slicewidth
                    c1_ += [0 for i in range(slicewidth-remainder)]
                for i in range(0, len(c1_), slicewidth):
    ...
    def write_delta_prime(self, delta_prime,
        ...
        with open(f_delta_prime, 'w') as f:
            ...
            if width is None:
                ...
                if rem != 0:
                    temp += '0'*(width - rem)
                for i in range(0, len(temp), width):
    ...
    def write_pubkey(self, T, f_pubkey=None, slicewidth=None):
        ...
        with open(f_pubkey, 'w') as f:
            if slicewidth is None:
                ...
                for slicestart in range(0, T.ncols(), slicewidth):
    ...
    def read_pubkey(self, f_pubkey=None, slicewidth=None):
        ...
        with open(f_pubkey, 'r') as f:
            lines = f.read().splitlines()
            T = matrix(GF(2), self.params.m*self.params.t, self.params.n-self.params.m*self.params.t)
            line_idx = 0
            column_width = slicewidth
            if slicewidth is None:
                for i in range(len(lines)):
                    T[i] = matrix(GF(2), [list(lines[i])])
            else:
                for column_offset in range(0, self.params.n-self.params.m*self.params.t, column_width):
                    for row_idx in range(T.nrows()):
                        row = matrix(GF(2), [list(lines[line_idx])])
                        row = row[:, :slicewidth:-1]
    ...
    def write_g(self, g, f_g=None, slicewidth=None):
        ...
        with open(f_g, 'w') as f:
            if slicewidth is None:
                for i in reversed(range(self.params.t+1)):
    ...
    def read_g(self, f_g=None, slicewidth=None):
        ...
        with open(f_g, 'r') as f:
            ...
            if slicewidth is None:
                ...
                line = line[:(self.params.t+1)*self.params.m]
            for offset in range(0, len(line), self.params.m):
                bin = line[offset:offset+self.params.m]
                c = self.params.Fq.fetch_int(int(bin, 2))
                beta.append(c)
            beta = beta[::-1]
            g = sum(beta[j]*x^j for j in range(self.params.t+1))
        return g
    ...
    def read_alphas(self, f_p=None):
        ...
        with open(f_p, 'r') as f:
            ...
            for bin in lines:
                bin = bin[::-1]
                alpha = self.params.Fq.fetch_int(int(bin, 2))
                alphas.append(alpha)
        return alphas
    ...
    def write_seed(self, seed=None, f_seed=None):
        ...
        if seed is None:
            seed = [[0 for i in range(32)] for j in range(16)]
            seed[15][7]=1
...
class McElieceKAT:
    ...
    def gen(self, delta=None):
        ...
        if delta is not None:
          delta = [int(i) for i in reversed(bin(delta)[2:])]
          delta = [0] * (self.params.l - len(delta)) + delta
        else:
...
if __name__ == "__main__":
    ...
    if args.generate:
        ...
        katio.write_delta_prime(delta_prime, width=32)
        ...
        katio.write_g(g, slicewidth=32)
        assert g == katio.read_g(slicewidth=32)
        ...
        C_ = katio.read_C(args.cipher[0], args.cipher[1])
        K_ = katio.read_K(args.sessionkey)

        # Read private key KAT files
        g_ = katio.read_g(slicewidth=32)
