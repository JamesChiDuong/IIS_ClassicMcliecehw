...
def decode(C,Gammaprime,params):
  ...
  assert len(Gammaprime) == n+_sage_const_1 
  g,alphaprime = Gammaprime[_sage_const_0 ],Gammaprime[_sage_const_1 :]
  ...
  v = list(C)+[_sage_const_0 ]*k
  v = list(vector(GF(_sage_const_2 ),v))
  ...
  c = vector(GF(_sage_const_2 ),[e[i]+v[i] for i in range(n)])

  # "If wt(e) = t and C = He, return e. Otherwise return False."
  if sum(_sage_const_1  if ej else _sage_const_0  for ej in e) != t: return False

  kpoly = g.parent()
  xminusalpha = [kpoly([-alphaprime[i],_sage_const_1 ]) for i in range(n)]
  A = kpoly(prod(xminusalpha))
  if sum(c[i]*A//xminusalpha[i] for i in range(n))%g != _sage_const_0 : return False
...
def test1():
  ...
  for system in parameters.alltests:
    ...
    g,alphaprime = Gamma[_sage_const_0 ],Gamma[_sage_const_1 :]
    ...
    if t > _sage_const_0 :
      while True:
        j = randrange(n)
        if not e[j]: continue
        edelta = vector(GF(_sage_const_2 ),[i==j for i in range(n)])
        Cmod = list(C)
        for i in range(m*t):
    ...
    C = vector(GF(_sage_const_2 ),[randrange(_sage_const_2 ) for j in range(m*t)])
