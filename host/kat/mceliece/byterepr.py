...
# This file was *autogenerated* from the file byterepr.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_8 = Integer(8); _sage_const_7 = Integer(7); _sage_const_2 = Integer(2); _sage_const_255 = Integer(255); _sage_const_4 = Integer(4); _sage_const_16 = Integer(16); _sage_const_15 = Integer(15); _sage_const_14 = Integer(14); _sage_const_100 = Integer(100)
...
def from_vector(v):
  ...
  assert all(vj in (_sage_const_0 ,_sage_const_1 ) for vj in v)

  r = len(v)
  expectedlen = ceil(r/_sage_const_8 )

  if r%_sage_const_8  != _sage_const_0 :
    ...
    padding = [_sage_const_0 ]*(_sage_const_8 -(r%_sage_const_8 ))
    assert len(padding) >= _sage_const_1 
    assert len(padding) <= _sage_const_7 
    v = v+padding
    assert len(v)%_sage_const_8  == _sage_const_0 
    # "and then represented as above"
    r = len(v)

  assert r == len(v)
  assert r%_sage_const_8  == _sage_const_0 
  assert expectedlen == r/_sage_const_8 
  ...
  b = [sum(int(v[j*_sage_const_8 +i])<<i for i in range(_sage_const_8 )) for j in range(r/_sage_const_8 )]
...
def to_vector(b,r,narrowly=True):
  ...
  b = list(bytearray(b))
  assert len(b) == ceil(r/_sage_const_8 )

  v = [_sage_const_1 &(bi>>j) for bi in b for j in range(_sage_const_8 )]
  assert len(v) == _sage_const_8 *len(b)

  if narrowly:
    # "Narrowly Decoded Classic McEliece rejects inputs ...
    #  where padding bits are nonzero"
    if any(vi != _sage_const_0  for vi in v[r:]):
...
def from_sessionkey(K,params):
  ...
  assert len(K) == l
  b = from_vector(K)
  assert len(b) == ceil(l/_sage_const_8 )
  return b
...
def from_ciphertext(C,params):
  ...
  assert len(C) == m*t
  b = from_vector(C)
  assert len(b) == ceil(m*t/_sage_const_8 )
  return b
...
def to_ciphertext(b,params):
  ...
  if parameters.supportpc:
    ...
    if params.pc:
      l = params.l
      b0,b1 = b[:ceil(m*t/_sage_const_8 )],b[ceil(m*t/_sage_const_8 ):]
...
def from_hashinput(x,params):
  ...
  if parameters.supportpc:
    # for testing against previous versions
    if params.pc:
      if x[_sage_const_0 ] == _sage_const_2 :
        assert len(x) == _sage_const_2 
        return bytes(bytearray([_sage_const_2 ]))+from_vector(x[_sage_const_1 ])
      u,v,C = x
      u = int(u)
      v = list(v)
  ...
  u,v,C = x
  u = int(u)
  assert u in (_sage_const_0 ,_sage_const_1 )
  v = list(v)
  assert len(v) == n
  ...
  assert bytearray(result)[_sage_const_0 ] in (_sage_const_0 ,_sage_const_1 )

  assert len(result) == _sage_const_1 +ceil(n/_sage_const_8 )+ceil(m*t/_sage_const_8 )
  return result
...
def from_publickey(T,params):
  ...
  assert len(b) == m*t*ceil(k/_sage_const_8 )
  return b
...
def to_publickey(b,params):
  ...
  k = params.k
  assert len(b) == m*t*ceil(k/_sage_const_8 )
  rows = [to_vector(b[j*ceil(k/_sage_const_8 ):(j+_sage_const_1 )*ceil(k/_sage_const_8 )],k) for j in range(m*t)]
  T = matrix(GF(_sage_const_2 ),rows)
  assert T.nrows() == m*t
  assert T.ncols() == k
  return T
...
def from_fieldelement(u,params):
  ...
  ubits = [_sage_const_1 &(uint>>j) for j in range(m)]
  b = from_vector(ubits)
  assert len(b) == ceil(m/_sage_const_8 )
  return b
...
def from_poly(g,params):
  ...
  assert len(b) == t*ceil(m/_sage_const_8 )
  return b
...
def to_poly(b,params):
  ...
  assert len(b) == t*ceil(m/_sage_const_8 )
  coeffs = [to_fieldelement(b[i*ceil(m/_sage_const_8 ):(i+_sage_const_1 )*ceil(m/_sage_const_8 )],params) for i in range(t)]
  g = Fqx(coeffs+[_sage_const_1 ])
  assert g.degree() == t
  assert g.is_monic()
  return g
...
def from_fieldordering(alpha,params):
  ...
  pi = [sum(((alphaint[i]>>(m-_sage_const_1 -j))&_sage_const_1 )<<j for j in range(m)) for i in range(q)]
  ...
  assert len(c) == (_sage_const_2 *m-_sage_const_1 )<<(m-_sage_const_1 )

  # "This vector is represented as ceil((2m-1)2^{m-4}) bytes as above."
  b = from_vector(c)
  assert len(b) == ceil((_sage_const_2 *m-_sage_const_1 )*_sage_const_2 **(m-_sage_const_1 )/_sage_const_8 )
...
def to_fieldordering(b,params):
  ...
  assert len(b) == ceil((_sage_const_2 *m-_sage_const_1 )*_sage_const_2 **(m-_sage_const_1 )/_sage_const_8 )
  c = to_vector(b,(_sage_const_2 *m-_sage_const_1 )<<(m-_sage_const_1 ))
  pi = controlbits.permutation(c)
  alpha = [Fq([_sage_const_1 &(pi[i]>>(m-_sage_const_1 -j)) for j in range(m)]) for i in range(q)]
...
def from_columnselection(c,params):
  ...
  assert all(cj <= m*t-mu+nu-_sage_const_1  for cj in c)

  # "represented as a ceil{nu/8}-byte string,
  #  the little-endian format of the integer
  #  sum_{i=0}^{mu-1} 2^{c_{mt-mu+i}-(mt-mu)}."
  cint = sum(_sage_const_2 **(ci-(m*t-mu)) for ci in c)
  b = from_vector([_sage_const_1 &(cint>>j) for j in range(nu)])
  assert len(b) == ceil(nu/_sage_const_8 )
  ...
  if (mu,nu) == (_sage_const_0 ,_sage_const_0 ):
    assert len(b) == _sage_const_0 
    b = bytes(bytearray([_sage_const_255 ,_sage_const_255 ,_sage_const_255 ,_sage_const_255 ,_sage_const_0 ,_sage_const_0 ,_sage_const_0 ,_sage_const_0 ]))
    assert len(b) == _sage_const_8 
...
def to_columnselection(b,params):
  ...
  if (mu,nu) == (_sage_const_0 ,_sage_const_0 ):
    assert b == bytes(bytearray([_sage_const_255 ,_sage_const_255 ,_sage_const_255 ,_sage_const_255 ,_sage_const_0 ,_sage_const_0 ,_sage_const_0 ,_sage_const_0 ]))
    return []

  assert len(b) == ceil(nu/_sage_const_8 )
  cvec = to_vector(b,nu)
  c = [m*t-mu+j for j in range(nu) if cvec[j] != _sage_const_0 ]
  assert len(c) == mu
  assert c == sorted(set(c))
  assert all(cj >= m*t-mu for cj in c)
  assert all(cj <= m*t-mu+nu-_sage_const_1  for cj in c)
...
def from_privatekey(priv,params):
  ...
  assert len(b0) == ceil(l/_sage_const_8 )
  ...
  assert len(b1) == _sage_const_8  if (mu,nu) == (_sage_const_0 ,_sage_const_0 ) else ceil(nu/_sage_const_8 )

  # "The t ceil(m/8)-byte string representing the polynomial g."
  b2 = from_poly(g,params)
  assert len(b2) == t*ceil(m/_sage_const_8 )

  # "The ceil((2m-1)2^(m-4)) bytes representing the field ordering alpha."
  b3 = from_fieldordering(alpha,params)
  assert len(b3) == ceil((_sage_const_2 *m-_sage_const_1 )*_sage_const_2 **(m-_sage_const_4 ))

  # "The ceil(n/8)-byte string representing s in F_2^n."
  assert len(s) == n
  b4 = from_vector(s)
  assert len(b4) == ceil(n/_sage_const_8 )
...
def to_privatekey(b,params):
  ...
  b0len = ceil(l/_sage_const_8 )
  b1len = _sage_const_8  if (mu,nu) == (_sage_const_0 ,_sage_const_0 ) else ceil(nu/_sage_const_8 )
  b2len = t*ceil(m/_sage_const_8 )
  b3len = ceil((_sage_const_2 *m-_sage_const_1 )*_sage_const_2 **(m-_sage_const_4 ))
  b4len = ceil(n/_sage_const_8 )
...
def test_vector():
  assert from_vector([_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ]) == b'\4'
  assert from_vector([_sage_const_0 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 , _sage_const_1 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_1 ,_sage_const_0 ]) == b'He'

  assert to_vector(b'He',_sage_const_16 ) == [_sage_const_0 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 , _sage_const_1 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_1 ,_sage_const_0 ]
  assert to_vector(b'He',_sage_const_15 ) == [_sage_const_0 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 , _sage_const_1 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_1 ]
  assert to_vector(b'He',_sage_const_14 ,narrowly=False) == [_sage_const_0 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 , _sage_const_1 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ]
  ok = False
  try:
    assert to_vector(b'He',_sage_const_14 ) == [_sage_const_0 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 , _sage_const_1 ,_sage_const_0 ,_sage_const_1 ,_sage_const_0 ,_sage_const_0 ,_sage_const_1 ]
  except:
    ok = True
  assert ok

  for r in range(_sage_const_100 ):
    print('bitvector %d' % r)
    sys.stdout.flush()
    v = [randrange(_sage_const_2 ) for i in range(r)]
    b = from_vector(v)
    assert len(b) == ceil(r/_sage_const_8 )
